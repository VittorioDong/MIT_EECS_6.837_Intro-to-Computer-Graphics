...
Vec3f RayTracer::RayCast(Ray &ray, float tmin, int bounces, float weight, Hit &hit)
{
    ...
    if (group->intersect(ray, hit, tmin))
    {
        ...
        Ray ref_ray;
        Vec3f decay;
        if (bounces < max_bounces && weight > min_weight && material->reflect(ray, hit, decay, ref_ray))
        {
        ...
        // 折射
        if (bounces < max_bounces && weight > min_weight && material->refract(ray, hit, decay, ref_ray))
        {
            Hit hit_ref(INFINITY);
            pixel_color += decay * RayCast(ref_ray, tmin, bounces + 1, weight * decay.Length(), hit_ref);
